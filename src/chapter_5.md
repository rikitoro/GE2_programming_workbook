...
どのようなものがあるかについては、テキスト資料を参照してください。
...
`val_output = (val_input > 0L) ? val_input : 0L;` の部分で
条件演算子 `?:` を用いています。
`(val_input > 0L) ? val_input : 0L` は、
条件 `val_input > 0L` が真 ( True ) であれば、
`val_input` を返し、偽 ( False ) であれば `0L` が返されます。
